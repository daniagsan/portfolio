...
// Animations restored based on original brutalist styles
...
const ServiceCard = ({ service, className }: { service: any; className?: string }) => {
  ...
  return (
    <motion.article
      ...
      className={`
        relative z-10 group cursor-pointer hover:z-50
    ...
    >
      ...
      <div className={`
        ...
        transform rotate-45 shadow-[6px_6px_0px_0px_rgba(0,0,0,1)]
        ${LEGACY_ANIMATIONS.container}
      `}>
        {/* Inner Content Container (Counter-rotated) */}
        <div className={`w-full h-full flex flex-col items-center justify-center p-2 sm:p-4 transform -rotate-45 ${LEGACY_ANIMATIONS.innerContainer}`}>

          {/* Active Content: Icon & Title */}
          <div className={`flex flex-col items-center -translate-y-2 sm:-translate-y-4 ${LEGACY_ANIMATIONS.iconWrapper}`}>
            ...
            {/* Title previously hidden by opacity-0 on hover */}
          ...
          <div className={`absolute inset-0 flex flex-col items-center justify-center p-3 sm:p-4 ${LEGACY_ANIMATIONS.hoverTextWrapper}`}>
